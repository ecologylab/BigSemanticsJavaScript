...
function checkForParams(selector, url){
    ...
    if (params !== undefined){
        ...
        for (var i in params){
            ...
            if (urlParams === undefined || !urlParams.hasOwnProperty(paramName))
                return false;
            var actualValue = urlParams[paramName];

            if (paramValue !== undefined && paramValue != actualValue)
                return false;

            if (actualValue === undefined)
                actualValue = "";
            if (curParam.isAllowEmptyValue && actualValue.length === 0){
                return false;
            }
        }
    }
