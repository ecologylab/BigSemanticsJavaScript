 ...
 *
 * @param container
 *   The HTML object to which the metadata rendering will be appended
 * @param url
 *   Url of the document you want to display metadata for
 * @param clipping
 *   Clippings either hold .rawMetadata, or have a pair of metadata and mmd. If
 *   the later, then MetadataLoader is never called!!!! If you already have
 *   metadata and mmd, please pass them in here in the form {metadata:
 *   your_data_here, mmd: you_data_here}
 * @param renderer
 *   The function used by a particular renderer to fill in the container with
 *   HTML
 * @param options
 *   object containing optional values, listend below:
 *   - requestMD, true if the function should request metadata from service,
 *     else false
 *   - reloadMD, true if the metadata should be extracted afresh vs being pulled
 *     from the cache (if possible), else false
 *   - isRoot, is this the root metadata for the rendering (currently used for
...
RendererBase.addMetadataDisplay = function(container, url, clipping, renderer, options) {
  ...
  if (clipping != null && clipping.metadata) {
    ...
    if (RendererBase.newBsService) {
      RendererBase.newBsService
        .loadMmd(clipping.metadata.mm_name, options)
        .then(function(mmd) {
          callback(undefined, mmd);
        })
        .catch(function(err) {
          callback(err);
        });
      ...
      bsService.onReady(function() {
        bsService.loadMmd(clipping.metadata.mm_name, options, callback);
      });
    }
  }
    ...
    if (!options.includeMmdInResult) {
      options.includeMmdInResult = true;
    }
    task.options = options;

    let bsProvider = undefined;
    let callback = function(err, md_and_mmd) {
      if (err) {
        console.error(err);
        return;
      }
      ...
      if (bsProvider) {
        if (bsProvider.constructor.name == "BSAutoSwitch") {
          console.log("loadMetadata result from " + bsProvider.bsImpl.constructor.name + ": ", md_and_mmd);
        }
        else {
          console.log("loadMetadata result from " + bsProvider.constructor.name + ": ", md_and_mmd);
        }
      }
    ...
    if (RendererBase.newBsService) {
      bsProvider = RendererBase.newBsService;
      RendererBase.newBsService
        .loadMetadata(url, options)
        .then(function(md_and_mmd) {
          callback(undefined, md_and_mmd);
        })
        .catch(function(err) {
          callback(err);
        });
...
DocumentContainer.prototype.matches = function(url) {
  ...
  for (var i = 0; i < this.urls.length; i++) {
    if (this.urls[i].length > 1) {
      return
      this.urls[i].localeCompare(url) == 0;
    }
  }
  return false;
}
