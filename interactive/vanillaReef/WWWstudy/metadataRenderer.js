				...
				{				
					...
					if( metadata[key].navigatesTo != null)
					{
						var citeULikeAtag = document.createElement('a');
							...
							citeULikeAtag.href = "http://www.citeulike.org/posturl?username="+ CITE_U_LIKE_USERNAME + "&bml=nopopup&url="+ metadata[key].navigatesTo +")";
						...
						fieldValueDiv.appendChild(citeULikeAtag);
		...
		if(valueCol != null)
		{
			...
			if(valueDiv != null)
			{
				...
				for (var j = 0; j < valueDiv.childNodes.length; j++)
				{
					...
					if(valueDiv.childNodes[j].href != null && valueDiv.childNodes[j].className != "citeULikeButton")
					{
						//console.log("my gosh");
...
var CITE_U_LIKE_USERNAME = "andruid";
